...
public class DefaultSynchronizeService extends DefaultService implements SynchronizeService {
	...
	public void synchronize() throws IOException {
		...
		PriorityQueue<ChangeRecord> orderedRemoteChanges = new PriorityQueue<ChangeRecord>(50,
		...
		for (ChangeRecord remoteChange : remoteChanges) {
			if (!localUploaded.containsKey(remoteChange.getLocalFile())) {
				...
				if (remoteChange.getOperation() == Operation.REMOTE_INSERT) {
					remoteChange.setLocalFile(stub.storage().remoteToLocal().get(remoteChange.getRemoteFileId()));
				}
	...
	private void addToSnapshot(Snapshot root, ChangeRecord remoteChange) {
		...
		case REMOTE_INSERT:
			...
			if (root.getName().equals(parent)) {
				...
				// Didn't find?
				logger.warn(MessageFormat.format("Remote insert cannot find local parent {0}", remoteChange));
		...
		case REMOTE_CHANGE: {
			...
			if (fileName.equals(root.getName())) {
				...
				// Didn't find?
				logger.error(MessageFormat.format("Local root doesn't contain this file {0}. Should be an error.",
						remoteChange));
				// throw new IllegalArgumentException();
			}
			break;
		...
		case REMOTE_RENAME: {
			...
			if (root.getName().startsWith(fileName)) {
				...
				logger.error(MessageFormat
						.format("Remote rename cannot find local corresponding {0}, should be an error", remoteChange));
				// throw new IllegalArgumentException();
			}
			break;
		}
	...
	private FailedRecord correct(FailedRecord fr) {
		if (fr.getOperation() == Operation.LOCAL_CHANGE || fr.getOperation() == Operation.LOCAL_RENAME) {
			...
			} else if (!StringUtils.isEmpty(fr.getError()) && fr.getError().startsWith("Local file doesn't exist")) {
				return null;
			}
		}
		if (fr.getOperation() == Operation.LOCAL_INSERT) {
			if (!StringUtils.isEmpty(fr.getError()) && fr.getError().startsWith("Local file doesn't exist")) {
				return null;
			}
		}
		if (fr.getOperation() == Operation.LOCAL_DELETE) {
			if (StringUtils.isEmpty(fr.getRemoteFileId()) || "404".equals(fr.getError())
					|| "403".equals(fr.getError())) {
				return null;
			}
		}
		if ("404".equals(fr.getError())
				&& (fr.getOperation() == Operation.LOCAL_CHANGE || fr.getOperation() == Operation.LOCAL_RENAME)) {
			// No such file
			fr.setOperation(Operation.LOCAL_INSERT);
			return fr;
		}
		if ("404".equals(fr.getError()) && (fr.getOperation() == Operation.REMOTE_INSERT
				|| fr.getOperation() == Operation.REMOTE_CHANGE || fr.getOperation() == Operation.REMOTE_RENAME
	...
	private void synchronize(ChangeRecord record) throws IOException {
		try {
			...
			case REMOTE_DELETE: {
				...
				if (localFile.exists()) {
					if (logger.isDebugEnabled()) {
						logger.debug(MessageFormat.format("Failed to delete file {0}. File may have been deleted.",
								record.getLocalFile()));
					}
				}
			...
			case REMOTE_RENAME: {
				...
				File newName = new File(local.getParentFile().getAbsolutePath() + File.separator + remoteName);
				...
				stub.storage().remoteToLocal().put(record.getRemoteFileId(), DriveUtils.relativePath(newName));
				stub.storage().localToRemote().remove(DriveUtils.relativePath(local));
				stub.storage().localToRemote().put(DriveUtils.relativePath(newName), record.getRemoteFileId());
				break;
			}
			}
		} catch (Exception e) {
	...
	private List<ChangeRecord> remoteChange() throws IOException {
		...
		for (Change remoteChange : changes) {
			...
			if (remoteChange.getDeleted()) {
				records.add(new ChangeRecord(Operation.REMOTE_DELETE, local, remoteChange.getFileId(),
				...
				records.add(new ChangeRecord(Operation.REMOTE_INSERT, null, remoteChange.getFileId(),
						remoteChange.getFile()));
			} else {
					...
					if (localFile.isFile()) {
						...
						if (localMd5.equals(remoteMd5)) {
							...
							if (!remoteName.equals(localFile.getName())) {
								records.add(new ChangeRecord(Operation.REMOTE_RENAME, local, remoteChange.getFileId(),
										remoteChange.getFile()));
							}
						} else {
							records.add(new ChangeRecord(Operation.REMOTE_CHANGE, local, remoteChange.getFileId(),
									remoteChange.getFile()));
						}
					} else {
						records.add(new ChangeRecord(Operation.REMOTE_INSERT, null, remoteChange.getFileId(),
	...
	protected List<ChangeRecord> compare(Snapshot standard, Snapshot current) {
		...
		if (standard.isFile() != current.isFile()) {
			throw new IllegalArgumentException(
					"Comparison between file and folder:" + standard.getName() + ":" + current.getName());
		}
		// The same
		...
		if (standard.isFile()) {
			changes.add(new ChangeRecord(Operation.LOCAL_CHANGE, standard.getName(),
					stub.storage().localToRemote().get(standard.getName())));
		} else {
			...
			for (Snapshot sc : standard.getChildren()) {
				if (!scMd5Table.containsKey(sc.getMd5Checksum())) {
					scMd5Table.put(sc.getMd5Checksum(), new ArrayList<Snapshot>());
				}
			...
			for (Snapshot cc : current.getChildren()) {
				if (!ccMd5Table.containsKey(cc.getMd5Checksum())) {
					ccMd5Table.put(cc.getMd5Checksum(), new ArrayList<Snapshot>());
				}
			...
			Collection<String> sameName = CollectionUtils.intersection(scNameTable.keySet(), ccNameTable.keySet());
			...
			for (String name : sameName) {
				...
				if (oldrec.isFile() != newrec.isFile()) {
					changes.add(new ChangeRecord(Operation.LOCAL_DELETE, oldrec.getName(),
							stub.storage().localToRemote().get(oldrec.getName())));
					changes.add(new ChangeRecord(Operation.LOCAL_INSERT, newrec.getName(), null));
				} else if (!oldrec.getMd5Checksum().equals(newrec.getMd5Checksum())) {
			...
			for (String newName : newNames) {
				...
				if (scMd5Table.containsKey(md5)) {
					...
					while (!oldrecs.isEmpty() && !newrecs.isEmpty()) {
						Snapshot oldrec = oldrecs.remove(0);
						Snapshot newrec = newrecs.remove(0);
						// Rename
						changes.add(new ChangeRecord(Operation.LOCAL_RENAME, oldrec.getName(),
								stub.storage().localToRemote().get(oldrec.getName()),
								DriveUtils.absolutePath(newrec.getName()).getName()));
					}
			...
			for (Entry<String, Snapshot> old : scNameTable.entrySet()) {
				changes.add(new ChangeRecord(Operation.LOCAL_DELETE, old.getValue().getName(),
						stub.storage().localToRemote().get(old.getValue().getName())));
			}

			// New files
			for (Entry<String, Snapshot> newe : ccNameTable.entrySet()) {
				changes.add(new ChangeRecord(Operation.LOCAL_INSERT, newe.getValue().getName(), null));
			}
		}
		return changes;
	}
